...
nnoremap <C-Space> <C-w>w        " Переключение между окнами (если <Ctrl+Space> не работает, замените на другое сочетание)
unmap <C-Space>                  " Отключение конфликтов, если <Ctrl+Space> уже используется
